...
<svelte:head>
  <title>Manage Sprints — Tazzina Admin</title>
</svelte:head>

<div class="space-y-6">
  <div class="flex items-center justify-between">
    <h2 class="text-xl font-semibold text-foreground">Sprints</h2>
    <form method="GET">
      <select
        name="team"
  ...
  {#if data.selectedTeamId}
    <form method="POST" action="?/create" use:enhance class="rounded-lg border border-border bg-card p-4 space-y-3">
      <input type="hidden" name="team_id" value={data.selectedTeamId} />
